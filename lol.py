...
# Extract the champion data from the API response
...
# Chunk the champion IDs into groups based on the number of columns
champion_chunks = [champion_ids[i:i + num_columns] for i in range(0, len(champion_ids), num_columns)]

# Create a table using the champion chunks
...
while True:
    ...
    if chosen_champion:
        ...
        # Make a GET request to retrieve the champion stats
        stats_response = requests.get(champion_stats_url)
        stats_data = stats_response.json()

        # Extract and display the desired stats
